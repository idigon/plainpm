...
2. Read `team/*.md` files to match owner names (use `first_name` for matching).
3. Read `projects/_index.md` to identify existing projects.
...
4. Determine the next task ID:
   - For stream tasks: scan ALL year folders under `projects/<slug>/streams/<stream>/tasks/` for existing IDs, increment from the highest found across all years
   - For project-level tasks: scan ALL year folders under `projects/<slug>/tasks/` for existing IDs, increment from the highest found across all years
...
6. If a new project is being created:
   - Create `projects/<slug>/project.md` from template
   - Add entry to `projects/_index.md`
...
7. Create the task file with proper YAML front matter:
   ...
   - Stream task path: `projects/<slug>/streams/<stream>/tasks/YYYY/TASK-ID.md`
   - Project-level task path: `projects/<slug>/tasks/YYYY/TASK-ID.md`
